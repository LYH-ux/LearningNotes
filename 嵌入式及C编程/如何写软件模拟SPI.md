...
typedef struct
{
    ...
    uint32_t ui32ClkGPIO;
    //
    //! The address of the GPIO pin to be used for the Tx signal. 
    uint32_t ui32TxGPIO;
    //
    //! The address of the GPIO pin to be used for the Rx signal.  If this
    //! member is zero, the Rx signal is not read.  
    uint32_t ui32RxGPIO;
...
1. ##### 关于GPIO
   ...
   GPIOBASE  占高5位十六进制数， GPIOPIN占一字节，8位。两个十六进制数。

   可先将两个地址合并设置，在设置GPIO时通过掩码来获取。 

   地址的掩码为 0xFFFFF000     引脚的掩码为 0x00000FFF.  

   引脚set时左移了两位，所以在最后掩码取余计算完后要右移两位。